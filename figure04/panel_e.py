...
if __name__ == '__main__': 
    ...
    #
#     fig_ang_w = np.round(mm_in_inch * 88.0, decimals=2)
#     fig_ang_h = np.round(mm_in_inch * 88.0, decimals=2)
#     fig_ang = plt.figure(3, figsize=(fig_ang_w, fig_ang_h))
#     fig_ang.clear()
#     fig_ang_x = left_margin/fig_ang_w
#     fig_ang_y = bottom_margin/fig_ang_h
#     fig_ang_w = 1.0 - (left_margin/fig_ang_w + right_margin/fig_ang_w)
#     fig_ang_h = 1.0 - (bottom_margin/fig_ang_h + top_margin/fig_ang_h)
#     ax_ang = fig_ang.add_axes([fig_ang_x, fig_ang_y, fig_ang_w, fig_ang_h])
#     ax_ang.clear()
...
#                          0 * ang[4:-4] + \
    ...
    for i_index in range(3):
        ...
        for i_bone in range(nJoints-1):
            ...
            ax3d.plot([joint_end[0]], [joint_end[1]], [joint_end[2]],
                    linestyle='', marker='.', markersize=2.5, markeredgewidth=0.5, color=color_joint, zorder=zorder_joint)
    dxyz = 22.5
    xyz_center = np.mean(skeleton_pos_mid, 0)
    ...
    ax3d.set_axis_off()
    ax3d.view_init(azim=-90, elev=30)
    # EDGES
    color = 'darkgray'
    gap_dx1 = 3.0
    gap_dx2 = -2.0
    ...
    for i_edge in range(2):
        ...
        ax3d.plot(x, y, color=color, zorder=0)
    fig3d.canvas.draw()
    ...
    if save:
        fig_xz.savefig(folder_save+'/trajectory_xz.svg',
#                          bbox_inches="tight",
                         dpi=300,
                         transparent=True,
                         format='svg',
                         pad_inches=0)
        fig_xy.savefig(folder_save+'/trajectory_xy.svg',
#                          bbox_inches="tight",
                         dpi=300,
                         transparent=True,
                         format='svg',
                         pad_inches=0)
        fig3d.savefig(folder_save+'/trajectory_3d.svg',
#                          bbox_inches="tight",
                         dpi=300,
                         transparent=True,
                         format='svg',
