...
#         acc_fit[1:-1] = (pos[2:, :, :, :2] - 2.0 * pos[1:-1, :, :, :2] + pos[:-2, :, :, :2]) / (1.0/200.0)**2
    ...
    for i_mode_pair in range(nPairs):
        ...
        wasserstein1 = scipy.stats.wasserstein_distance(pos0[:, :, :2][mask1[:, :, :2]].ravel(), pos1[:, :, :2][mask1[:, :, :2]].ravel())
        wasserstein2 = scipy.stats.wasserstein_distance(pos0[:, :, :2][mask2[:, :, :2]].ravel(), pos2[:, :, :2][mask2[:, :, :2]].ravel())
        print('position')
        ...
        hist_acc1 = np.histogram(acc1, bins=nBin_acc, range=[0.0, maxBin_acc0], normed=None, weights=None, density=False)
        hist_velo1 = np.histogram(velo1, bins=nBin_velo, range=[0.0, maxBin_velo0], normed=None, weights=None, density=False)
        hist_pos1 = np.histogram(pos1, bins=nBin_pos, range=[0.0, maxBin_pos0], normed=None, weights=None, density=False)
        hist_ang1 = np.histogram(ang1, bins=nBin_ang, range=[0.0, maxBin_ang0], normed=None, weights=None, density=False)
        hist_acc2 = np.histogram(acc2, bins=nBin_acc, range=[0.0, maxBin_acc0], normed=None, weights=None, density=False)
        hist_velo2 = np.histogram(velo2, bins=nBin_velo, range=[0.0, maxBin_velo0], normed=None, weights=None, density=False)
        hist_pos2 = np.histogram(pos2, bins=nBin_pos, range=[0.0, maxBin_pos0], normed=None, weights=None, density=False)
        hist_ang2 = np.histogram(ang2, bins=nBin_ang, range=[0.0, maxBin_ang0], normed=None, weights=None, density=False)
...
#         mode = 'asymp'
        ...
        print('avg.:\t\t{:0.8f}\t{:0.8f}'.format(np.mean(acc1*conversion_fac_acc), np.mean(acc2*conversion_fac_acc)))
...
#         print('median:\t\t{:0.8f}\t{:0.8f}'.format(median_acc1, median_acc2))
        print('median:\t\t{:0.8f}\t{:0.8f}'.format(np.median(acc1*conversion_fac_acc), np.median(acc2*conversion_fac_acc)))
        ...
        print('avg.:\t\t{:0.8f}\t{:0.8f}'.format(np.mean(velo1*conversion_fac_velo), np.mean(velo2*conversion_fac_velo)))
        print('sd.:\t\t{:0.8f}\t{:0.8f}'.format(np.std(velo1*conversion_fac_velo), np.std(velo2*conversion_fac_velo)))
#         print('median:\t\t{:0.8f}\t{:0.8f}'.format(median_velo1, median_velo2))
        print('median:\t\t{:0.8f}\t{:0.8f}'.format(np.median(velo1*conversion_fac_velo), np.median(velo2*conversion_fac_velo)))
        print('max.:\t\t{:0.8f}\t{:0.8f}'.format(np.max(velo1*conversion_fac_velo), np.max(velo2*conversion_fac_velo)))
        print('min.:\t\t{:0.8f}\t{:0.8f}'.format(np.min(velo1*conversion_fac_velo), np.min(velo2*conversion_fac_velo)))
        ...
        print('avg.:\t\t{:0.8f}\t{:0.8f}'.format(np.mean(pos1*conversion_fac_pos), np.mean(pos2*conversion_fac_pos)))
...
#         print('median:\t\t{:0.8f}\t{:0.8f}'.format(median_pos1, median_pos2))
        print('median:\t\t{:0.8f}\t{:0.8f}'.format(np.median(pos1*conversion_fac_pos), np.median(pos2*conversion_fac_pos)))
        ...
        print('avg.:\t\t{:0.8f}\t{:0.8f}'.format(np.mean(ang1*conversion_fac_ang), np.mean(ang2*conversion_fac_ang)))
...
#         print('median:\t\t{:0.8f}\t{:0.8f}'.format(median_ang1, median_ang2))
        print('median:\t\t{:0.8f}\t{:0.8f}'.format(np.median(ang1*conversion_fac_ang), np.median(ang2*conversion_fac_ang)))
        ...
        hist_acc1_use = np.histogram(acc1, bins=nBin_acc, range=[bin_range_acc[0], bin_range_acc[-1]], normed=None, weights=None, density=False)
        hist_velo1_use = np.histogram(velo1, bins=nBin_velo, range=[bin_range_velo[0], bin_range_velo[-1]], normed=None, weights=None, density=False)
        hist_pos1_use = np.histogram(pos1, bins=nBin_pos, range=[bin_range_pos[0], bin_range_pos[-1]], normed=None, weights=None, density=False)
        hist_ang1_use = np.histogram(ang1, bins=nBin_ang, range=[bin_range_ang[0], bin_range_ang[-1]], normed=None, weights=None, density=False)
        hist_acc2_use = np.histogram(acc2, bins=nBin_acc, range=[bin_range_acc[0], bin_range_acc[-1]], normed=None, weights=None, density=False)
        hist_velo2_use = np.histogram(velo2, bins=nBin_velo, range=[bin_range_velo[0], bin_range_velo[-1]], normed=None, weights=None, density=False)
        hist_pos2_use = np.histogram(pos2, bins=nBin_pos, range=[bin_range_pos[0], bin_range_pos[-1]], normed=None, weights=None, density=False)
        hist_ang2_use = np.histogram(ang2, bins=nBin_ang, range=[bin_range_ang[0], bin_range_ang[-1]], normed=None, weights=None, density=False)
...
#         x_ang2[2::2] = np.copy(hist_ang2[1][1:])
        ...
        if (show or save):
            if not(modes_ploted[i_mode1]):
                ...
                ax_acc_all.plot(x_acc1*conversion_fac_acc, y_acc1, color=colors[i_mode1], linestyle='-', marker='', zorder=-4+mode_pair[0],
                                linewidth=linewidth_hist, clip_on=False)
                ax_velo_all.plot(x_velo1*conversion_fac_velo, y_velo1, color=colors[i_mode1], linestyle='-', marker='', zorder=-4+mode_pair[0],
                                linewidth=linewidth_hist, clip_on=False)
                ax_pos_all.plot(x_pos1*conversion_fac_pos, y_pos1, color=colors[i_mode1], linestyle='-', marker='', zorder=-4+mode_pair[0],
                                linewidth=linewidth_hist, clip_on=False)
                ax_ang_all.plot(x_ang1*conversion_fac_ang, y_ang1, color=colors[i_mode1], linestyle='-', marker='', zorder=-4+mode_pair[0],
                                linewidth=linewidth_hist, clip_on=False)
            if not(modes_ploted[i_mode2]):
                modes_ploted[i_mode2] = True
                ax_acc_all.plot(x_acc2*conversion_fac_acc, y_acc2, color=colors[i_mode2], linestyle='-', marker='', zorder=-4+mode_pair[1],
                                linewidth=linewidth_hist, clip_on=False)
                ax_velo_all.plot(x_velo2*conversion_fac_velo, y_velo2, color=colors[i_mode2], linestyle='-', marker='', zorder=-4+mode_pair[1],
                                linewidth=linewidth_hist, clip_on=False)
                ax_pos_all.plot(x_pos2*conversion_fac_pos, y_pos2, color=colors[i_mode2], linestyle='-', marker='', zorder=-4+mode_pair[1],
                                linewidth=linewidth_hist, clip_on=False)
                ax_ang_all.plot(x_ang2*conversion_fac_ang, y_ang2, color=colors[i_mode2], linestyle='-', marker='', zorder=-4+mode_pair[1],
                                linewidth=linewidth_hist, clip_on=False)
...
#         text.set_fontname(fontname)
    ax_acc_all.set_xlabel('acceleration ({:s})'.format(acc_unit), va='bottom', ha='center', fontsize=fontsize, fontname=fontname)
    ax_acc_all.set_ylabel('probability', va='top', ha='center', fontsize=fontsize, fontname=fontname)
    ax_acc_all.set_xlim([(bin_range_acc[0]-offset_x_fac*dBin_acc)*conversion_fac_acc, (bin_range_acc[-1]+offset_x_fac*dBin_acc)*conversion_fac_acc])
    ax_acc_all.set_xticks([0.0, maxBin_acc0*conversion_fac_acc*0.5, maxBin_acc0*conversion_fac_acc, maxBin_acc*conversion_fac_acc])
...
#     ax_acc_all.ticklabel_format(axis='both', style='sci', scilimits=(0, 0), useMathText=True)    
    ...
    for tick in ax_acc_all.get_yticklabels():
        tick.set_fontname(fontname)
        tick.set_fontsize(fontsize)
    ax_acc_all.xaxis.set_label_coords(x=ax_acc_all_x+0.5*ax_acc_all_w, y=bottom_margin_x/fig_acc_all_h, transform=fig_acc_all.transFigure)
    ax_acc_all.yaxis.set_label_coords(x=left_margin_x/fig_acc_all_w, y=ax_acc_all_y+0.5*ax_acc_all_h, transform=fig_acc_all.transFigure)
    #
    ...
    ax_acc_all.plot(np.array([-line_w*0.5, +line_w*0.5], dtype=np.float64) + maxBin_acc0*conversion_fac_acc*(1.0+inf_fac*0.5) - box_w*0.5,
                    np.array([-line_h*0.5, +line_h*0.5], dtype=np.float64) + ax_acc_all.get_ylim()[0],
                    linewidth=ax_acc_all.spines['left'].get_linewidth(), linestyle='-', color='black', zorder=101, clip_on=False)
    ax_acc_all.plot(np.array([-line_w*0.5, +line_w*0.5], dtype=np.float64) + maxBin_acc0*conversion_fac_acc*(1.0+inf_fac*0.5) + box_w*0.5,
                    np.array([-line_h*0.5, +line_h*0.5], dtype=np.float64) + ax_acc_all.get_ylim()[0],
                    linewidth=ax_acc_all.spines['bottom'].get_linewidth(), linestyle='-', color='black', zorder=101, clip_on=False)
    box = plt.Rectangle((maxBin_acc0*conversion_fac_acc*(1.0+inf_fac*0.5) - box_w*0.5, ax_acc_all.get_ylim()[0]+abs(np.diff(ax_acc_all.get_ylim()))*0.5-box_h*0.5),
                         box_w, box_h,
...
#     for text in h_legend.get_texts():
#         text.set_fontname(fontname)
    ax_velo_all.set_xlabel('velocity ({:s})'.format(velo_unit), va='bottom', ha='center', fontsize=fontsize, fontname=fontname)
    ax_velo_all.set_ylabel('probability', va='top', ha='center', fontsize=fontsize, fontname=fontname)
    ax_velo_all.set_xlim([(bin_range_velo[0]-offset_x_fac*dBin_velo)*conversion_fac_velo, (bin_range_velo[-1]+offset_x_fac*dBin_velo)*conversion_fac_velo])
    ax_velo_all.set_xticks([0.0, maxBin_velo0*conversion_fac_velo*0.5, maxBin_velo0*conversion_fac_velo, maxBin_velo*conversion_fac_velo])
...
#     ax_velo_all.ticklabel_format(axis='both', style='sci', scilimits=(0, 0), useMathText=True)
    ...
    for tick in ax_velo_all.get_yticklabels():
        tick.set_fontname(fontname)
        tick.set_fontsize(fontsize)
    ax_velo_all.xaxis.set_label_coords(x=ax_velo_all_x+0.5*ax_velo_all_w, y=bottom_margin_x/fig_velo_all_h, transform=fig_velo_all.transFigure)
    ax_velo_all.yaxis.set_label_coords(x=left_margin_x/fig_velo_all_w, y=ax_velo_all_y+0.5*ax_velo_all_h, transform=fig_velo_all.transFigure)
    #
    ...
    ax_velo_all.plot(np.array([-line_w*0.5, +line_w*0.5], dtype=np.float64) + maxBin_velo0*conversion_fac_velo*(1.0+inf_fac*0.5) - box_w*0.5,
                     np.array([-line_h*0.5, +line_h*0.5], dtype=np.float64) + ax_velo_all.get_ylim()[0],
                     linewidth=ax_velo_all.spines['left'].get_linewidth(), linestyle='-', color='black', zorder=101, clip_on=False)
    ax_velo_all.plot(np.array([-line_w*0.5, +line_w*0.5], dtype=np.float64) + maxBin_velo0*conversion_fac_velo*(1.0+inf_fac*0.5) + box_w*0.5,
                     np.array([-line_h*0.5, +line_h*0.5], dtype=np.float64) + ax_velo_all.get_ylim()[0],
                     linewidth=ax_velo_all.spines['bottom'].get_linewidth(), linestyle='-', color='black', zorder=101, clip_on=False)
    box = plt.Rectangle((maxBin_velo0*conversion_fac_velo*(1.0+inf_fac*0.5) - box_w*0.5, ax_velo_all.get_ylim()[0]+abs(np.diff(ax_velo_all.get_ylim()))*0.5-box_h*0.5),
                         box_w, box_h,
                         color='white', zorder=100, clip_on=False)
...
#     for text in h_legend.get_texts():
#         text.set_fontname(fontname)
    ax_pos_all.set_xlabel('position error ({:s})'.format(pos_unit), va='bottom', ha='center', fontsize=fontsize, fontname=fontname)
    ax_pos_all.set_ylabel('probability', va='top', ha='center', fontsize=fontsize, fontname=fontname)
    ax_pos_all.set_xlim([(bin_range_pos[0]-offset_x_fac*dBin_pos)*conversion_fac_pos, (bin_range_pos[-1]+offset_x_fac*dBin_pos)*conversion_fac_pos])
    ax_pos_all.set_xticks([0.0, maxBin_pos0*conversion_fac_pos*0.5, maxBin_pos0*conversion_fac_pos, maxBin_pos*conversion_fac_pos])
...
#     ax_pos_all.ticklabel_format(axis='y', style='sci', scilimits=(0, 0), useMathText=True)
    ...
    for tick in ax_pos_all.get_yticklabels():
        tick.set_fontname(fontname)
        tick.set_fontsize(fontsize)
    ax_pos_all.xaxis.set_label_coords(x=ax_pos_all_x+0.5*ax_pos_all_w, y=bottom_margin_x/fig_pos_all_h, transform=fig_pos_all.transFigure)
    ax_pos_all.yaxis.set_label_coords(x=left_margin_x/fig_pos_all_w, y=ax_pos_all_y+0.5*ax_pos_all_h, transform=fig_pos_all.transFigure)
    #
    ...
    ax_pos_all.plot(np.array([-line_w*0.5, +line_w*0.5], dtype=np.float64) + maxBin_pos0*conversion_fac_pos*(1.0+inf_fac*0.5) - box_w*0.5,
                    np.array([-line_h*0.5, +line_h*0.5], dtype=np.float64) + ax_pos_all.get_ylim()[0],
                    linewidth=ax_pos_all.spines['left'].get_linewidth(), linestyle='-', color='black', zorder=101, clip_on=False)
    ax_pos_all.plot(np.array([-line_w*0.5, +line_w*0.5], dtype=np.float64) + maxBin_pos0*conversion_fac_pos*(1.0+inf_fac*0.5) + box_w*0.5,
                    np.array([-line_h*0.5, +line_h*0.5], dtype=np.float64) + ax_pos_all.get_ylim()[0],
                    linewidth=ax_pos_all.spines['bottom'].get_linewidth(), linestyle='-', color='black', zorder=101, clip_on=False)
    box = plt.Rectangle((maxBin_pos0*conversion_fac_pos*(1.0+inf_fac*0.5) - box_w*0.5, ax_pos_all.get_ylim()[0]+abs(np.diff(ax_pos_all.get_ylim()))*0.5-box_h*0.5),
                         box_w, box_h,
                         color='white', zorder=100, clip_on=False)
...
#     for text in h_legend.get_texts():
#         text.set_fontname(fontname)
    ax_ang_all.set_xlabel('angle error ({:s})'.format(ang_unit), va='bottom', ha='center', fontsize=fontsize, fontname=fontname)
    ax_ang_all.set_ylabel('probability', va='top', ha='center', fontsize=fontsize, fontname=fontname)
    ax_ang_all.set_xlim([(bin_range_ang[0]-offset_x_fac*dBin_ang)*conversion_fac_ang, (bin_range_ang[-1]+offset_x_fac*dBin_ang)*conversion_fac_ang])
    ax_ang_all.set_xticks([0.0, maxBin_ang0*conversion_fac_ang*0.5, maxBin_ang0*conversion_fac_ang, maxBin_ang*conversion_fac_ang])
...
#     ax_ang_all.ticklabel_format(axis='y', style='sci', scilimits=(0, 0), useMathText=True)
    ...
    for tick in ax_ang_all.get_yticklabels():
        tick.set_fontname(fontname)
        tick.set_fontsize(fontsize)
    ax_ang_all.xaxis.set_label_coords(x=ax_ang_all_x+0.5*ax_ang_all_w, y=bottom_margin_x/fig_ang_all_h, transform=fig_ang_all.transFigure)
    ax_ang_all.yaxis.set_label_coords(x=left_margin_x/fig_ang_all_w, y=ax_ang_all_y+0.5*ax_ang_all_h, transform=fig_ang_all.transFigure)
    #
    ...
    ax_ang_all.plot(np.array([-line_w*0.5, +line_w*0.5], dtype=np.float64) + maxBin_ang0*conversion_fac_ang*(1.0+inf_fac*0.5) - box_w*0.5,
                    np.array([-line_h*0.5, +line_h*0.5], dtype=np.float64) + ax_ang_all.get_ylim()[0],
                    linewidth=ax_pos_all.spines['left'].get_linewidth(), linestyle='-', color='black', zorder=101, clip_on=False)
    ax_ang_all.plot(np.array([-line_w*0.5, +line_w*0.5], dtype=np.float64) + maxBin_ang0*conversion_fac_ang*(1.0+inf_fac*0.5) + box_w*0.5,
                    np.array([-line_h*0.5, +line_h*0.5], dtype=np.float64) + ax_ang_all.get_ylim()[0],
                    linewidth=ax_pos_all.spines['bottom'].get_linewidth(), linestyle='-', color='black', zorder=101, clip_on=False)
    box = plt.Rectangle((maxBin_ang0*conversion_fac_ang*(1.0+inf_fac*0.5) - box_w*0.5, ax_ang_all.get_ylim()[0]+abs(np.diff(ax_ang_all.get_ylim()))*0.5-box_h*0.5),
                         box_w, box_h,
                         color='white', zorder=100, clip_on=False)
