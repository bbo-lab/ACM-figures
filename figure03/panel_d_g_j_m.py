...
#         index_use = np.argmax(pos_avg[i_paw_timing]) # end of limb swing phase
    ...
    #         range_y_use = np.array([range_y_min-range_diff*0.5, range_y_max+range_diff*0.5], dtype=np.float64)
        ...
        if save:
            for i_folder in range(nFolders):
                fig1.savefig(folder_save+'/gait_position__population__{:s}.svg'.format(paws_joint_names[i_paw_timing]),
    #                          bbox_inches="tight",
                             dpi=300,
                             transparent=True,
                             format='svg',
    ...
    #     ax2.plot(time, np.full(len(time), threshold_velocity, dtype=np.float64), linestyle='-', marker='', linewidth=linewidth color='black', alpha=0.5, zorder=10) # also plot velocity threshold
        ...
        if save:
            for i_folder in range(nFolders):
                fig2.savefig(folder_save+'/gait_velocity__population__{:s}.svg'.format(paws_joint_names[i_paw_timing]),
    #                          bbox_inches="tight",
                             dpi=300,
                             transparent=True,
                             format='svg',
    ...
    # #     if save:
    # #         for i_folder in range(nFolders):
    # #             fig3.savefig(folder_save+'/gait_acceleration__population__{:s}.svg'.format(paws_joint_names[i_paw_timing]),
    # #                          bbox_inches="tight",
    # #                          dpi=300,
    # #                          transparent=True,
    # #                          format='svg',
    # #                          pad_inches=0)
        ...
        for i_ang_pair in range(nAngles_pairs):
            ...
            if save:
                for i_folder in range(nFolders):
                    fig4[i_ang_pair].savefig(folder_save+'/gait_angle__population__{:s}.svg'.format(paws_joint_names[i_paw_timing]),
#                                              bbox_inches="tight",
                                             dpi=300,
                                             transparent=True,
                                             format='svg',
        ...
        for i_ang_pair in range(nAngles_pairs):
            ...
            if save:
                for i_folder in range(nFolders):
                    fig41[i_ang_pair].savefig(folder_save+'/gait_angle_velocity__population__{:s}.svg'.format(paws_joint_names[i_paw_timing]),
#                                              bbox_inches="tight",
                                             dpi=300,
                                             transparent=True,
                                             format='svg',
    ...
    # #     # detection # not fullt complete yet (i.e. marker names)
    # #     fig9 = plt.figure(3, figsize=(14, 7))
    # #     fig9.clear()
    ...
    # #     ax9.spines["right"].set_visible(False) 
    # #     for i_paw in range(i_paw_plot_start, i_paw_plot_end+1):
    # #         ax9.plot(time, detec_avg[i_paw], linestyle='-', marker='', color=colors_paws[i_paw], linewidth=linewidth, alpha=1.0, zorder=i_paw+1+4)
    # #     for i_paw in range(i_paw_plot_start, i_paw_plot_end+1): # for correct legend order
    # #         ax9.fill_between(x=time, y1=detec_avg[i_paw]-detec_std[i_paw], y2=detec_avg[i_paw]+detec_std[i_paw],
    # #                          color=colors_paws[i_paw], linewidth=linewidth,
    # #                          alpha=0.2, zorder=i_paw+1,
    # #                          where=None, interpolate=False, step=None, data=None)
    # #     ax9.set_xlim([time[0]-(1.0/frame_rate)*offset_x_fac*time_fac, time[-1]+(1.0/frame_rate)*offset_x_fac*time_fac])
    # #     h_legend = ax9.legend(paws_joint_names_legend[i_paw_plot_start:i_paw_plot_end+1], loc='upper right', fontsize=fontsize, frameon=False)
    # #     for text in h_legend.get_texts():
    # #         text.set_fontname(fontname)
    # #     ax9.set_xlabel('time (ms)', fontsize=fontsize, fontname=fontname)
    # #     ax9.set_ylabel('avg. detection'.format(axis_s), fontsize=fontsize, fontname=fontname)
    # #     fig9.canvas.draw()
    # #     if save:
    # #         for i_folder in range(nFolders):
    # #             fig9.savefig(folder_save+'/gait_detection__population__{:s}.svg'.format(paws_joint_names[i_paw_timing]),
    # #                          bbox_inches="tight",
    # #                          dpi=300,
    # #                          transparent=True,
    # #                          format='svg',
    # #                          pad_inches=0)
        ...
        if save:
            for i_folder in range(nFolders):
                fig5.savefig(folder_save+'/gait_position_limb__population__{:s}.svg'.format(paws_joint_names[i_paw_timing]),
                             bbox_inches="tight",
                             dpi=300,
                             transparent=True,
                             format='svg',
    ...
    #     ax6.plot(time, np.full(len(time), threshold_velocity, dtype=np.float64), linestyle='-', marker='', linewidth=linewidth, color='black', alpha=0.5, zorder=10) # also plot velocity threshold
        ...
        if save:
            for i_folder in range(nFolders):
                fig6.savefig(folder_save+'/gait_velocity_limb__population__{:s}.svg'.format(paws_joint_names[i_paw_timing]),
                             bbox_inches="tight",
                             dpi=300,
                             transparent=True,
                             format='svg',
    ...
    # #     if save:
    # #         for i_folder in range(nFolders):
    # #             fig7.savefig(folder_save+'/gait_acceleration_limb__population__{:s}.svg'.format(paws_joint_names[i_paw_timing]),
    # #                          bbox_inches="tight",
    # #                          dpi=300,
    # #                          transparent=True,
    # #                          format='svg',
    ...
    #         range_y_use = np.array([range_y_min-range_diff*0.5, range_y_max+range_diff*0.5], dtype=np.float64)
        ...
        if save:
            for i_folder in range(nFolders):
                fig8.savefig(folder_save+'/gait_angle_limb__population__{:s}.svg'.format(paws_joint_names[i_paw_timing]),
    #                          bbox_inches="tight",
                             dpi=300,
                             transparent=True,
                             format='svg',
